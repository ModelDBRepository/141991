...
prnt1=fopen("Model1_STA_epsg_epsp_iklt.dat","w");
fclose(prnt1);
prnt2=fopen("Model1_train_EPSGs-EPSPs.dat","wt");
prnt3=fopen("Model1_ITD-curve.dat","w");
fclose(prnt3);
...
for(o=0;o<=Npoints;o++){
	...
	for(h=0;h<Ntrials;h++){
		...
		while(t<tTOT){
			...
			gLT=1.3*q3p03*GLT*(v[4]*v[4]*v[4]*v[4])*v[5];
	...
	prnt1=fopen("Model1_STA_epsg_epsp_iklt.dat","a");
	p=0;
	for(p=0;p<N_steps_sta;p++){
	...
	prnt3=fopen("Model1_ITD-curve.dat","a");
